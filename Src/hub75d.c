...
static void DispLunarCalendar(struct RgbType *rgb, u8 i)
{
    ...
    u8 m1 = (lcData.month & 0x70) >> 4;
    u8 m0 = lcData.month & 0x0F;
    u8 d1 = (lcData.day & 0x70) >> 4;
    u8 d0 = lcData.day & 0x0F;
...
void HUB75D_DispScan(void)
{
    ...
    DispTime(&calendarDecimal, &rgb, count);
    if (count < 8) {
        DispDate(&calendarDecimal, &rgb, count);
    } else {
...
{
    struct CalendarDecimal *caleDeci;

    caleDeci->hourL = time->hour % 10;
    caleDeci->hourH = time->hour / 10;
    caleDeci->minL = time->min % 10;
    caleDeci->minH = time->min / 10;
    caleDeci->secL = time->sec % 10;
    caleDeci->secH = time->sec / 10;

    caleDeci->dayL = time->day % 10;
    caleDeci->dayH = time->day / 10;
    caleDeci->monthL = time->month % 10;
    caleDeci->monthH = time->month / 10;
    caleDeci->yearL = time->year % 100 % 10;
    caleDeci->yearH = time->year % 100 / 10;
    caleDeci->week = time->week;
    caleDeci->colon = 11; /* ":" */
}
