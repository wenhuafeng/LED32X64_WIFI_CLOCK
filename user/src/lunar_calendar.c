...
static bool LunarCalendar(struct TimeType *time, struct LunarCalendarType *lcData)
{
    ...
    if (((g_lunarCalendarTable[year - 1901] & 0x0060) >> 5) == 1) {
        ...
        springToNY = (g_lunarCalendarTable[year - 1901] & 0x001F) - 1 + 31;
    }
    sunToNY = g_monthAdd[month - 1] + day - 1;
    if (((year % 4) == 0) && (month > 2)) {
        sunToNY++;
    }

    if (sunToNY >= springToNY) { /* 阳历日在春节后（含春节那天） */
        sunToNY -= springToNY;
        month = 1;
        index = 1;
        flag = 0;
        ...
        while (sunToNY >= dayCount) {
            sunToNY -= dayCount;
            index++;
            if (month == ((g_lunarCalendarTable[year - 1901] & 0xF00000) >> 20)) {
                flag = ~flag;
                if (flag == 0) {
                    month++;
                }
        ...
        year--;
        month = 12;
        if (((g_lunarCalendarTable[year - 1901] & 0xF00000) >> 20) == 0) {
            index = 12;
        } else {
            index = 13;
        }
